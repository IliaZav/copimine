...
def test_binding_fix_is_cache_busted_on_auth_and_link_pages():
    assert 'from "./auth/auth-page.js?v=20260902route1"' in BOOTSTRAP
    assert 'from "./shared/app-routes.js?v=20260902route1"' in BOOTSTRAP
    for relative in ("signin.html", "register.html"):
        html = (ROOT / "admin-web/frontend" / relative).read_text(encoding="utf-8")
        assert '/assets/app.js?v=20260902route1' in html
...
def test_launcher_link_runtime_cache_key_changes_when_binding_flow_changes():
    ...
    assert 'app.js?v=20260902route1' in link
    assert 'js/bootstrap.js?v=20260902route1' in app
    assert 'cabinet-runtime.js?v=20260902route1' in bootstrap
