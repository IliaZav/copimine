...
usage() { printf 'Usage: sudo bash %s /path/to/release.tar.gz [sha256] [--wipe-db]\n' "$0" >&2; }
[[ -n "$ARCHIVE_PATH" ]] || { usage; exit 2; }
if [[ "${2:-}" == "--wipe-db" ]]; then ARCHIVE_SHA256=""; WIPE_DB=1; fi
if [[ "${3:-}" == "--wipe-db" ]]; then WIPE_DB=1; fi
...
exec "$PROJECT_ROOT/deploy/ubuntu/copimine_unpack_and_verify.sh" "$ARCHIVE_PATH" "$ARCHIVE_SHA256"
