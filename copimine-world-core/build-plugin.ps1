...
$jar = Join-Path $pluginDir 'CopiMineWorldCore.jar'
$serverJar = Join-Path $serverDir 'plugins\CopiMineWorldCore.jar'
$serverDataDir = Join-Path $serverDir 'plugins\CopiMineWorldCore'
...
if (-not $sources -or $sources.Count -eq 0) {
  throw 'No Java sources found for CopiMineWorldCore.'
}
...
if ($LASTEXITCODE -ne 0) { throw "javac failed for CopiMineWorldCore." }
...
if ($LASTEXITCODE -ne 0) { throw "jar packaging failed for CopiMineWorldCore." }
