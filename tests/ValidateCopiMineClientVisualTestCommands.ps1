...
foreach ($needle in @('literal("shader")','literal("restore")','startLocalShaderTest')) {
  if ($client -notmatch [regex]::Escape($needle)) { throw "Client shader test command marker missing: $needle" }
}
