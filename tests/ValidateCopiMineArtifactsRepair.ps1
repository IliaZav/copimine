...
foreach ($marker in @('openRepair(','executeRepair(','artifact_repairs','repairPrice(','repair:confirm:')) {
  if ($text -notmatch [regex]::Escape($marker)) { $errors.Add("Missing repair marker: $marker") }
}
if ($text -notmatch 'artifact[-_]repair[-_]refund') { $errors.Add('Missing repair marker: artifact_repair_refund') }
