...
Require-Contains $text 'routePollingStationInteractAsync(player, protectedInfo.linkedId())' 'RP stations must route to the async direct-vote loader.'
...
Require-Regex $text 'private void openDirectVoteMenuAsync\(Player player, String stationId\) \{[\s\S]*runAsync\(\(\) -> \{[\s\S]*isDirectVotingOpen\(station\)[\s\S]*activeCandidates\(string\(station\.get\("election_id"\)\), round\)' 'Direct voting must load station state, stage, voter status, and candidates asynchronously.'
