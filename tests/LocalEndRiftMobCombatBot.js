...
function attackNearest() {
  ...
  bot.lookAt(target.position.offset(0, 0.8, 0), true).then(() => {
    ...
    bot.attack(refreshed)
    attackCount += 1
    console.log(`PLAYER_ATTACK ${username} count=${attackCount} target=${refreshed.id} type=${refreshed.name} distance=${distance(refreshed.position, bot.entity.position).toFixed(2)}`)
...
bot.once('spawn', () => {
  ...
  positionTimer = setInterval(() => {
    if (bot.entity) bot._client.write('position', { x: bot.entity.position.x, y: bot.entity.position.y, z: bot.entity.position.z, onGround: true })
  }, 100)
  sampleTimer = setInterval(sampleMobs, 250)
  attackTimer = setInterval(attackNearest, 750)
...
bot.on('end', () => {
  for (const timer of [positionTimer, sampleTimer, attackTimer, healthTimer]) if (timer !== null) clearInterval(timer)
