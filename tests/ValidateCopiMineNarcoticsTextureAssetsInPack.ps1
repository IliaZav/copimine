...
try {
  foreach ($entry in @(
    ...
    if (-not ($zip.Entries | Where-Object { $_.FullName -eq $entry })) { throw "Missing narcotics pack entry: $entry" }
  }
  foreach ($entry in @(
    ...
    if (-not ($zip.Entries | Where-Object { $_.FullName -eq $entry })) { throw "Missing narcotics texture/overlay pack entry: $entry" }
  }
} finally {
  $zip.Dispose()
}
Write-Host 'Narcotics texture assets in pack validation passed.'
