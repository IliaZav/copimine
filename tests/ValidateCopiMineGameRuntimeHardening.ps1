...
if (Test-Path -LiteralPath $policy) {
    ...
    Require-Contains $policyText '"Enabled": true' 'ImageFrame URL restriction must be enabled in the managed policy.'
    ...
    Require-Contains $policyText '"default": 0' 'Default players must have no ImageFrame creation allowance.'
    Require-Contains $policyText '"admin": -1' 'Admin-only ImageFrame creation must remain possible.'
