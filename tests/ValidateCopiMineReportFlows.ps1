...
Require-Contains $pluginJava 'PendingBugReport pending=pendingBugReports.get(p.getUniqueId())' 'Reporta flow must check pending bug context.'
...
Require-Regex $pluginJava 'notifyPlayerBug[\s\S]*warn\(player,"Error code:.*Type:' 'Player-facing bug message must expose code and short English summary only.'
