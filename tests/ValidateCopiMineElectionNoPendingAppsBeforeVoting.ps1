...
if ($null -eq $stateMachine) {
  ...
  Require-Contains $stateMachine 'long pendingApplications = scalarLong(connection, "SELECT COUNT(*) FROM candidate_applications WHERE election_id=? AND admin_status=' 'State machine must count pending candidate applications from DB.'
