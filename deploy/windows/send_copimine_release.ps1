...
$remoteCheck = "cd '$RemoteDir' && expected=`$(awk '{print tolower(`$1)}' '$archiveName.sha256' | head -n1) && actual=`$(sha256sum '$archiveName' | awk '{print tolower(`$1)}') && test `"`$expected`" = `"`$actual`""
