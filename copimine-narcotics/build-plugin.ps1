...
$jar = Join-Path $pluginDir 'CopiMineNarcotics.jar'
$serverJar = Join-Path $serverDir 'plugins\CopiMineNarcotics.jar'
$serverDataDir = Join-Path $serverDir 'plugins\CopiMineNarcotics'
...
foreach ($dependency in @(
  (Join-Path $releaseRoot 'copimine-economy-core\CopiMineEconomyCore.jar'),
  (Join-Path $releaseRoot 'copimine-admin-plugin\CopiMineUltimateAdminPlus.jar'),
  (Join-Path $releaseRoot 'copimine-election-core\CopiMineElectionCore.jar'),
...
$sources = Get-ChildItem -Path $srcDir -Recurse -Filter '*.java' | ForEach-Object FullName
if (-not $sources -or $sources.Count -eq 0) {
  throw 'No Java sources found for CopiMineNarcotics.'
}
...
if ($LASTEXITCODE -ne 0) { throw "javac failed for CopiMineNarcotics." }
...
Write-Host "Copied default config to $serverDataDir"
