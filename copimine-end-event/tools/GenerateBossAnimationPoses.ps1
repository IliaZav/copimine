...
$mappedBones = [ordered]@{
  ...
  CHEST = 'body'
  PELVIS = 'body'
  LEFT_UPPER_ARM = 'left_hand'
  LEFT_FOREARM = 'left_hand_low'
  RIGHT_UPPER_ARM = 'right_hand'
  RIGHT_FOREARM = 'right_hand_low'
  LEFT_LEG = 'left_leg'
  RIGHT_LEG = 'right_leg'
}
$clips = @(
...
foreach ($clipSpec in $clips) {
  ...
  if ($null -ne $animation.bones) {
    foreach ($boneProperty in @($animation.bones.PSObject.Properties | Sort-Object Name)) {
      ...
      if ($mappedBones.Values -contains $boneName) {
        $parsedTrack = [ordered]@{
...
foreach ($clip in $parsedClips) {
  $entries = @($clip.Tracks.GetEnumerator() | Sort-Object Key | ForEach-Object {
      $track = $_.Value
      $entryLine = '                "' + $_.Key + '", new BoneTrack(' + (Render-Channel $track.Rotation) + ', ' + (Render-Channel $track.Position) + ', ' + (Render-Channel $track.Scale) + ')'
      $entryLine
    })
