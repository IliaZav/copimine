...
if ($backend -notmatch '"deop"\s*:\s*f"deop \{player\}"') { throw 'Player access controls do not expose the deop command.' }
if ($backend -notmatch 'f"deop \{minecraft_name\}"' -or $backend -notmatch 'f"lp user \{minecraft_name\} parent remove admin"' -or $backend -notmatch 'f"lp user \{minecraft_name\} parent remove junior_admin"' -or $backend -notmatch 'f"lp user \{minecraft_name\} parent remove senior_admin"') { throw 'Demotion is missing OP/LuckPerms revocation.' }
