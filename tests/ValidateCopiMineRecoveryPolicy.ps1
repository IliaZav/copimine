...
$donationIdentityMatch = [regex]::Match($artifacts, '(?s)private CopiMineArtifacts\.OfficialDonationRef officialDonationRef\(ItemStack var1\).*?(?=\n\s+private )')
...
Require-Contains $donationIdentity 'this.isDonationCatalogItem(var6)' 'Donation identity must accept only donation catalog items.'
