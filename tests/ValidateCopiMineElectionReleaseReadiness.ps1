...
Require-Contains $text 'setStage(requireActiveElectionId(), requestedStage, player.getName(),' 'GUI stage actions must validate and route through setStage.'
...
Require-Regex $text 'if \(action\.startsWith\("apply:stage:"\)\)[\s\S]*setStage\(requireActiveElectionId\(\), requestedStage, player\.getName\(\),' 'apply:stage:* must call setStage rather than direct SQL.'
