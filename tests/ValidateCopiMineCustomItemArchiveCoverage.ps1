...
if ($ids.Count -lt 20 -or @($ids | Select-Object -Unique).Count -ne $ids.Count) { $errors.Add('Texture source mapping must contain at least 20 unique custom-textured catalog ids including the AR infinite totem; the vanilla shield is intentionally excluded.') }
foreach ($row in @($sources.items)) {
  ...
  if ($row.catalog -eq 'ADMIN_ONLY' -and $row.source_group -notin @('User_Supplied', 'Generated')) { $errors.Add("Wrong source group for admin-only item $($row.id)") }
