...
function Wait-PostgresReady {
    ...
    for ($attempt = 1; $attempt -le $Attempts; $attempt++) {
        ...
        # PostgreSQL may accept TCP before crash recovery is complete.  These
        # messages are transient readiness states, not configuration errors.
        if ($lastOutput -notmatch '(?i)database system is starting up|database system is not yet accepting connections|consistent recovery state has not been reached|could not connect to server|connection refused|server closed the connection unexpectedly') {
...
function Sync-LocalFirstPartyPlugins {
    $pluginPairs = @(
        ...
        @{ Source = (Join-Path $Root 'copimine-end-event\CopiMineEndEvent.jar'); Runtime = (Join-Path $ServerDir 'plugins\CopiMineEndEvent.jar') }
    )
