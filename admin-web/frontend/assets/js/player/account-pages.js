export function createPlayerAccountPages(deps) {
  ...
  async function loadPlayerSettings() {
    ...
    setView(`
      <section class="layout-grid grid-2">
        ...
        ${panel("Что можно настроить", "В кабинете остаются только реальные действия без пустых переключателей и служебного мусора.", safetyRail([
  ...
  async function loadPlayerSecurity() {
    ...
    setView(`
      <section class="layout-grid grid-2">
        ...
        ${panel("Сессии", "Базовые действия безопасности без скрытых служебных шагов.", `
  ...
  async function loadPlayerSupport() {
    ...
    setView(`
      <section class="layout-grid grid-2">
        ${panel("Как обратиться за помощью", "Здесь собраны только реальные способы связи, которые уже работают на сервере.", safetyRail([
        ...
        ${panel("Частые вопросы", "Короткие ответы по самым частым ситуациям после входа.", safetyRail([
