...
if ($statusText -notmatch 'state=.*(?:UNLOCKED|UNCONFIGURED)') {
  throw "Recovery smoke expected an unlocked or safely unconfigured state. Actual status: $statusText"
}
Write-Host 'PASS recovery state=UNLOCKED|UNCONFIGURED'
foreach ($expected in @('endUnlocked=.*true', 'VICTORY_COMPLETE')) {
...
if (Test-Path -LiteralPath $LogPath -PathType Leaf) {
  ...
  if (-not $hasPersistedPhase -or (-not $hasForcedUnlock -and -not $hasForcedUnconfigured -and -not $hasAlreadyUnlocked)) {
