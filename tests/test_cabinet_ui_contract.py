...
def test_cabinet_public_header_uses_the_same_forest_shell_as_public_pages() -> None:
    ...
    for path in cabinet_templates():
        source = path.read_text(encoding="utf-8")
        assert "/assets/cabinet.css?v=20260831cabinetui25" in source, path.name
        assert "/assets/js/cabinet-polish.js?v=20260831cabinetui25" in source, path.name
